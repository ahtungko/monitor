    ...
    function addblock(resp) {
        ...
        const template = (item) => {
            ...
            return `
                ...
                <div id='vpsInfo'>
                <span>Cookie状态：</span> <h5 style='color: ${ckState(item[7])[1]}'>${item[2]} : ${ckState(item[7])[0]}</h5>
                <span>过期时间：</span> <h5 style="color: #fef50c; font-size: 14px">${utcStr}</h5>
                <span>最近查询时间：</span> <h5 style="color: blue;">${item[6]}</h5>
                <span>位置：</span> <h5>${item[5]}</h5>
